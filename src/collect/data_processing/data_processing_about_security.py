...
def match_from_end(s: str) -> str:
    match = re.search(r"(Security-related|Security-unrelated)", s)
    if match:
...
if __name__ == "__main__":
    ...
    gpt_check_task = Task(
        name="filter-security-related",
        system=(
            "You will be given a task instance concerning analyzing"
            " problems of cellular network protocol. The sample is composed"
            " of three parts, SYSTEM INSTRUCTION, USER QUERY, and ASSISTANT"
            " RESPONSE. You should determine whether the implied problem is high-risk, meaning that it strongly relates to"
            " security, user privacy, attacks, or any threats to normal service.\n"
            "You should meticulously analyze the given task instance and end up"
            " with a judgement. If the problem discussed by the instance is high-risk,"
            " you should finally respond with 'Security-related'; otherwise, respond with 'Security-unrelated'."
        ),
        shots=None,
    ...
    if process_args.llm_checking:
        ...
        sr_sources_list = [
            source for source, yes_or_no in results if yes_or_no == "Seurity-related"
        ]
        sur_sources_list = [
            source for source, yes_or_no in results if yes_or_no == "Security-unrelated"
        ]
