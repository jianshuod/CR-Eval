...
def get_ftp_path_and_download(wg_tdoc, output_dir):
    url = f"https://netovate.com/doc-search/?fname={wg_tdoc}"
...
def apply_multiprocessing(wg_tdoc_list, func, output_dir, num_processes):
    with multiprocessing.Pool(num_processes) as pool:
        tqdm.pandas()
        ftp_paths = list(
            tqdm(
                pool.imap(func, [(wg_tdoc, output_dir) for wg_tdoc in wg_tdoc_list]),
                total=len(wg_tdoc_list),
            )
        )
    return ftp_paths
